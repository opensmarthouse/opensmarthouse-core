...
public class AnnotatedThingActionModuleTypeProviderTest extends JavaTest {
    ...
    @Before
    public void setUp() {
        mockThing1 = mock(Thing.class);
        when(mockThing1.getUID()).thenReturn(new ThingUID(TEST_THING_TYPE_UID, "test1"));
        mockHandler1 = mock(ThingHandler.class);
        when(mockHandler1.getThing()).thenReturn(mockThing1);

        actionProviderConf1 = new TestThingActionProvider();
        actionProviderConf1.setThingHandler(mockHandler1);

        mockThing2 = mock(Thing.class);
        when(mockThing2.getUID()).thenReturn(new ThingUID(TEST_THING_TYPE_UID, "test2"));
        mockHandler2 = mock(ThingHandler.class);
        when(mockHandler2.getThing()).thenReturn(mockThing2);
